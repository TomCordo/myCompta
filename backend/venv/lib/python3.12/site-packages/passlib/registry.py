...
def register_crypt_handler_path(name, path):
    ...
    for a class named ``myhash`` within the ``mycompta.helpers`` module::

        >>> from passlib.registry import registry_crypt_handler_path
        >>> registry_crypt_handler_path("myhash", "mycompta.helpers")

    ...while this form would cause ``get_handler("myhash")`` to look
    for a class name ``MyHash`` within the ``mycompta.helpers`` module::

        >>> from passlib.registry import registry_crypt_handler_path
        >>> registry_crypt_handler_path("myhash", "mycompta.helpers:MyHash")
    """
    # validate name
